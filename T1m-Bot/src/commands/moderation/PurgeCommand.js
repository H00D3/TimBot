...
module.exports = class PurgeCommand extends BaseCommand {
  ...
  async run(client, message, args) {
    if (!message.member.hasPermission("MANAGE_MESSAGES")) return message.channel.send('You cannot use this command homie.');
    if (!message.guild.me.hasPermission("MANAGE_MESSAGES")) return message.channel.send("I do not have \`MANAGE_MESSAGES\` permission.");
    if (!args[0]) return message.channel.send("You must state a number of messages to be deleted. \`>purge number\`");
    ...
    try {
      await message.channel.bulkDelete(fetched)
       .then(message.channel.send(`**Deleted ${messages.size} messages!**`));
    } catch (err) {
      console.log(err);
      message.channel.send(`I was unable to delete the ammount stated make sure they are within 14 days old.`);
